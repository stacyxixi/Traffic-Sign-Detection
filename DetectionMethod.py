...
def classify_ROI_red(img_in, img):
    ...
    id1, val1, p1 = result1
    # "stopsign match at Roi{} position{} at value{}".format(id1,p1,val1)
    ...
    if val1 > 0.35 and val1 + 0.14 == max(val2, (val1 + 0.14), val3):
        pos1 = pos_list[id1]
        pos_s.append(pos1)
        results['stop'] = pos_s
...
def detect_ROI_red(img_in, img):
    ...
    #cv2.imwrite("output/test/stop/{}_original.png".format(img[1][0:18]), img_copy)
...
def green_light_detection(img_in, img, radii_range):
    ...
    for roi in roi_list:
        roi_id += 1
        #print roi.shape
        #(os.path.join(images_path, "green_roi_{}.png".format(roi_id)), roi)
